use crate::{
    format::{GENERATED_HEADER_LINES as HEADER_LINES, LEGACY_GENERATED_HEADER_LINES, SEPARATOR},
    shell::{shell_quote, shell_word},
};

const UTF8_BOM: char = '\u{feff}';

pub(crate) fn restore(text: &str) -> String {
    let mut result = Vec::new();
    let mut lines = strip_generated_header(text).lines().peekable();

    while let Some(line) = lines.next() {
        if line.starts_with("# gitignore.in format: ") {
            continue;
        }
        if line == SEPARATOR {
            let Some(section_head) = lines.next() else {
                break;
            };

            if let Some(target) = section_head.strip_prefix("# gibo dump ") {
                result.push(format!("gibo dump {}", shell_word(target)));
                skip_generated_section(&mut lines);
                continue;
            }

            if let Some(target) = section_head.strip_prefix("# gi ") {
                result.push(format!("gi {}", shell_word(target)));
                skip_generated_section(&mut lines);
                continue;
            }

            if section_head == SEPARATOR {
                continue;
            }

            result.push(format!("echo {}", shell_quote(section_head)));

            continue;
        }

        if line.is_empty() {
            result.push(line.to_string());
            continue;
        }

        if let Some(comment) = restore_comment_line(line) {
            result.push(comment);
            continue;
        }

        result.push(format!("echo {}", shell_quote(line)));
    }

    result.join("\n") + "\n"
}

pub(crate) fn looks_generated(text: &str) -> bool {
    let line_set: std::collections::HashSet<&str> = text.lines().collect();
    HEADER_LINES.iter().all(|header| line_set.contains(header))
        || LEGACY_GENERATED_HEADER_LINES
            .iter()
            .all(|header| line_set.contains(header))
}

fn strip_generated_header(text: &str) -> &str {
    let text = text.strip_prefix(UTF8_BOM).unwrap_or(text);
    strip_exact_header(text, &HEADER_LINES)
        .or_else(|| strip_exact_header(text, &LEGACY_GENERATED_HEADER_LINES))
        .unwrap_or(text)
}

fn strip_exact_header<'a>(text: &'a str, header: &[&str]) -> Option<&'a str> {
    let mut remaining = text;
    for expected in header {
        let (line, rest) = split_first_line(remaining)?;
        if line != *expected {
            return None;
        }
        remaining = rest;
    }
    Some(remaining)
}

fn split_first_line(text: &str) -> Option<(&str, &str)> {
    if text.is_empty() {
        return None;
    }
    if let Some(line_end) = text.find('\n') {
        Some((&text[..line_end], &text[line_end + 1..]))
    } else {
        Some((text, ""))
    }
}

fn skip_generated_section<'a, I>(lines: &mut std::iter::Peekable<I>)
where
    I: Iterator<Item = &'a str>,
{
    while let Some(next) = lines.peek() {
        if *next == SEPARATOR {
            break;
        }
        lines.next();
    }
}

// Decode the double-hash encoding that build() writes for comment lines.
// build() stores comments as `Comment::Content("# original")` and emits
// `# # original` (prefixing `# ` once more).  Stripping the outer `# `
// here recovers the original `# original` line from .gitignore.in.
fn restore_comment_line(line: &str) -> Option<String> {
    line.strip_prefix("# #")
        .map(|comment| format!("#{comment}"))
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn embedded_header_substrings_do_not_trigger_generated() {
        // All header strings appear as substrings within longer lines, but not as
        // exact lines. looks_generated must return false so full inference is used.
        let text = concat!(
            "# NOTE: DO NOT EDIT THIS FILE directly\n",
            "# Generated by gitignore.in (custom build)\n",
            "# See https://gitignore.in/ for more info\n",
            "# Edit .gitignore.in instead of this file (if desired)\n",
            "# Run `gitignore.in` to build .gitignore (optional)\n",
        );
        assert!(!looks_generated(text));
    }

    #[test]
    fn legacy_four_line_header_is_recognized_as_generated() {
        // v0.2.0 generated files had 4 header lines (no "# See https://gitignore.in/").
        // looks_generated must return true so restore() is chosen over the heuristic infer path.
        let text = concat!(
            "# DO NOT EDIT THIS FILE\n",
            "# Generated by gitignore.in\n",
            "# Edit .gitignore.in instead of this file\n",
            "# Run `gitignore.in` to build .gitignore\n",
        );
        assert!(looks_generated(text));
    }

    #[test]
    fn restores_generated_gitignore() {
        let text = r#"# DO NOT EDIT THIS FILE
# Generated by gitignore.in
# See https://gitignore.in/
# Edit .gitignore.in instead of this file
# Run `gitignore.in` to build .gitignore
# # project comment
# -----------------------------------------------------------------------------
# gibo dump Linux
### Generated by gibo
foo
# -----------------------------------------------------------------------------
# gi Rust
### Created by gitignore.io
bar
# -----------------------------------------------------------------------------
!.env
"#;

        let expected = "# project comment\ngibo dump Linux\ngi Rust\necho '!.env'\n";
        assert_eq!(restore(text), expected);
    }

    #[test]
    fn preserves_manual_header_comments_when_input_is_not_generated() {
        let text = concat!(
            "# Generated by gitignore.in\n",
            "# See https://gitignore.in/\n",
            "manual-entry\n",
        );

        let expected =
            "echo '# Generated by gitignore.in'\necho '# See https://gitignore.in/'\necho 'manual-entry'\n";
        assert_eq!(restore(text), expected);
    }

    #[test]
    fn restores_generated_gitignore_with_utf8_bom() {
        let text = concat!(
            "\u{feff}# DO NOT EDIT THIS FILE\n",
            "# Generated by gitignore.in\n",
            "# See https://gitignore.in/\n",
            "# Edit .gitignore.in instead of this file\n",
            "# Run `gitignore.in` to build .gitignore\n",
            "# # project comment\n",
            "# -----------------------------------------------------------------------------\n",
            "# gi Rust\n",
            "### Created by gitignore.io\n",
            "target\n",
            "# -----------------------------------------------------------------------------\n",
            "!.env\n",
        );

        let expected = "# project comment\ngi Rust\necho '!.env'\n";
        assert_eq!(restore(text), expected);
    }

    #[test]
    fn quotes_echo_content() {
        let text = "# -----------------------------------------------------------------------------\na'b\n";
        assert_eq!(restore(text), "echo 'a'\\''b'\n");
    }

    #[test]
    fn restores_mixed_content_sections() {
        let text = r#"# DO NOT EDIT THIS FILE
# Generated by gitignore.in
# See https://gitignore.in/
# Edit .gitignore.in instead of this file
# Run `gitignore.in` to build .gitignore
# # keep this comment
# -----------------------------------------------------------------------------
plain-entry
# -----------------------------------------------------------------------------
!important.txt
# trailing note
"#;

        let expected =
            "# keep this comment\necho 'plain-entry'\necho '!important.txt'\necho '# trailing note'\n";
        assert_eq!(restore(text), expected);
    }

    #[test]
    fn round_trips_generated_comment_shape() {
        let restored = restore("# # original comment\n");
        assert_eq!(restored, "# original comment\n");
    }

    #[test]
    fn restores_empty_echo() {
        // separator + empty line comes from `echo ''` in .gitignore.in
        let text =
            "# -----------------------------------------------------------------------------\n\n";
        assert_eq!(restore(text), "echo ''\n");
    }

    #[test]
    fn quotes_multiword_gibo_target() {
        let text = concat!(
            "# DO NOT EDIT THIS FILE\n",
            "# Generated by gitignore.in\n",
            "# See https://gitignore.in/\n",
            "# Edit .gitignore.in instead of this file\n",
            "# Run `gitignore.in` to build .gitignore\n",
            "# -----------------------------------------------------------------------------\n",
            "# gibo dump Visual Studio\n",
            "### Generated by gibo\n",
            "foo\n",
            "# -----------------------------------------------------------------------------\n",
        );
        let restored = restore(text);
        assert_eq!(restored, "gibo dump 'Visual Studio'\n");
    }

    #[test]
    fn quotes_multiword_gi_target() {
        let text = concat!(
            "# DO NOT EDIT THIS FILE\n",
            "# Generated by gitignore.in\n",
            "# See https://gitignore.in/\n",
            "# Edit .gitignore.in instead of this file\n",
            "# Run `gitignore.in` to build .gitignore\n",
            "# -----------------------------------------------------------------------------\n",
            "# gi Visual Studio\n",
            "### Created by gitignore.io\n",
            "bar\n",
            "# -----------------------------------------------------------------------------\n",
        );
        let restored = restore(text);
        assert_eq!(restored, "gi 'Visual Studio'\n");
    }

    #[test]
    fn format_version_line_is_not_echoed_during_restore() {
        // The format-version marker from .gitignore.in header must be absorbed during restore,
        // not emitted as `echo '# gitignore.in format: v1'` in the restored .gitignore.in.
        let text = concat!(
            "# DO NOT EDIT THIS FILE\n",
            "# Generated by gitignore.in\n",
            "# See https://gitignore.in/\n",
            "# Edit .gitignore.in instead of this file\n",
            "# Run `gitignore.in` to build .gitignore\n",
            "# gitignore.in format: v1\n",
            "# -----------------------------------------------------------------------------\n",
            "plain-entry\n",
        );
        let restored = restore(text);
        assert!(
            !restored.contains("# gitignore.in format: v1"),
            "{restored}"
        );
        assert!(restored.contains("echo 'plain-entry'"), "{restored}");
    }

    #[test]
    fn consecutive_separators_produce_no_echo() {
        // A manually-edited .gitignore may contain two adjacent separators.
        // The second separator must be skipped, not emitted as `echo '# ---...---'`.
        let text = concat!(
            "# -----------------------------------------------------------------------------\n",
            "# -----------------------------------------------------------------------------\n",
        );
        assert_eq!(restore(text), "\n");
    }

    #[test]
    fn round_trips_empty_echo_via_build() {
        // build("echo ''\n") produces separator + empty line;
        // restore should recover "echo ''\n" from that output.
        use crate::assembler::build;
        use crate::parser::parse_text;
        let input = "echo ''\n";
        let script = parse_text(input);
        let built = build(script).unwrap();
        let restored = restore(&built);
        assert_eq!(restored, input);
    }
}
